...
*/
class JuceCLAPWrapper  : public AudioProcessorListener,
...
public:
    ...
    bool getCurrentPosition (AudioPlayHead::CurrentPositionInfo& info) override { return false; }
...
#endif
    //==============================================================================
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JuceCLAPWrapper)
};

clap_plugin_descriptor JuceCLAPWrapper::desc = {CLAP_VERSION,
                                         "fix.me.with.id",
                                         JucePlugin_Name,
...
namespace ClapAdapter {
    ...
    const clap_plugin_descriptor *clap_get_plugin_descriptor(uint32_t w) { return &JuceCLAPWrapper::desc; }

    static const clap_plugin *clap_create_plugin(const clap_host *host, const char *plugin_id) {
        if (strcmp(plugin_id, JuceCLAPWrapper::desc.id)) {
            std::cout << "Warning: CLAP asked for plugin_id '" << plugin_id << "' and JuceCLAPWrapper ID is '"
